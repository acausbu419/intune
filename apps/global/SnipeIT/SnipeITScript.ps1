...
if($snipeit_check -ne $null){
    if(!(Test-Path -Path 'HKLM:\Software\ARcare\AutoPilot\')){
        ...
        New-ItemProperty -Path 'HKLM:\Software\ARcare\AutoPilot\' -Name SnipeIT2021 -Value 1 -Force
    }
    else{New-ItemProperty -Path 'HKLM:\Software\ARcare\AutoPilot\' -Name SnipeIT2021 -Value 1 -Force}
    Exit
}
else{
    ...
    if(!(Test-Path -Path 'HKLM:\Software\ARcare\AutoPilot\')){
        New-Item -Path 'HKLM:\Software\ARcare' -ItemType key
        New-Item -Path 'HKLM:\Software\ARcare\AutoPilot' -ItemType key
        New-ItemProperty -Path 'HKLM:\Software\ARcare\AutoPilot\' -Name SnipeIT -Value 1 -Force
    }
